...
async def on_ready():
    ...
    await client.change_presence(activity=discord.Game("Devloped by Gamer Galore"))
...
async def bankfee():
    while True:
        db = sqlite3.connect("main.sqlite")
        cursor = db.cursor()
        cursor.execute(f"SELECT user_id FROM eco")
        result = cursor.fetchone()

        cursor.execute(f"SELECT user_id, balance, deposited, bankrupt, deposit_limit FROM eco WHERE user_id = {member.id}")
        eco = cursor.fetchone()
        try:
            user_id = eco[0]
            user_bal = eco[1]
        ...
        if result is None:
            sql = ("INSERT INTO eco(user_name, user_id, balance, deposited, bankrupt, deposit_limit) VALUES (?, ?, ?, ?, ?, ?)")
            val = (f"{user.global_name}", user.id, 100, 0, "Not Bankrupt", 100)
            cursor.execute(sql, val)
        db.commit()
...
async def welcome(ctx):
    ...
    info_embed.add_field(name="auto role", value="Set automatic role so when a user joins they will recieve it automatically.", inline=False)
