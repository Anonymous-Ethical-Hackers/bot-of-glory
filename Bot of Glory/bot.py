...
#config file
if os.path.exists(os.getcwd() + "./db/json/config.json"):
    ...
    with open(os.getcwd() + "./db/json/config.json", "w+") as f:
...
#eco.json file
if os.path.exists(os.getcwd() + "./db/json/eco.json"):
    pass
else:
    ecoTemplate = {}

    with open(os.getcwd() + "./db/json/eco.json", "w+") as f:
        json.dump(ecoTemplate, f)

#welcome.json file
if os.path.exists(os.getcwd() + "./db/json/welcome.json"):
    pass
else:
    welcome_Template = {}

    with open(os.getcwd() + "./db/json/welcome.json", "w+") as f:
        json.dump(welcome_Template, f)
...
if os.path.exists(os.getcwd() + "./db/json/welcome.json"):
    ...
    with open(os.getcwd() + "./db/json/welcome.json", "w+") as f:
        json.dump(bank_Template, f)

#mute.json file
if os.path.exists(os.getcwd() + "./db/json/mute.json"):
    pass
else:
    mute_Template = {}

    with open(os.getcwd() + "./db/json/mute.json", "w+") as f:
        json.dump(mute_Template, f)
...
async def balance(ctx, member: discord.Member=None):
    ...
    if str(member.id) not in user_eco:
        ...
        user_eco[str(member.id)]["Bankrupt"] = "Not Bankrupt"
        user_eco[str(member.id)]["Deposit Limit"] = 100

        with open("db/json/eco.json", "w") as f:
            json.dump(user_eco, f, indent=4)
        
    user_bal = user_eco[str(member.id)]["Balance"]
    user_dp = user_eco[str(member.id)]["Deposited"]
    user_bankrupt = user_eco[str(member.id)]["Bankrupt"]
    user_dp_limit = user_eco[str(member.id)]["Deposit Limit"]

    if user_eco[str(member.id)]["Bankrupt"] == "Bankrupt":
        eco_red_embed = discord.Embed(title=f"{member.name}'s Currnet Balance", description="The current balance of this user.", color=discord.Color.red())
        ...
        eco_red_embed.set_footer(text="Want to incress balance? Try running some economy based commands!", icon_url=None)
...
async def beg(ctx):
    ...
    if new_bal < -99:
        ...
        user_eco[str(ctx.author.id)]["Bankrupt"] = "Bankrupt" 

        with open("db/json/eco.json", "w") as f:
            json.dump(user_eco, f, indent=4)

        bank[str(ctx.author.id)] = {}

        with open("db/json/bank.json", "w") as f:
            json.dump(bank, f, indent=4)

        if str(ctx.author.id) in bank:
            bank_depo = user_eco[str(ctx.author.id)]["Deposited"]
            ...
            no_beg_em.add_field(name="Your deposited is:", value=f"Deposited: ${bank_depo}")
...
async def steal(ctx, member: discord.Member):
    ...
    if steal_probability == 1: #user gets steal
        ...
        if user_eco[str(member.id)]["Balance"] < amount:
            return await ctx.send(f"Sorry {member.mention} does not have ${amount}. Sorry you cannont steal from them to right now.")
        
        elif user_eco[str(member.id)]["Balance"] >= amount:
            user_eco[str(ctx.author.id)]["Balance"] += amount
...
#deposit
@commands.cooldown(1, 280, commands.BucketType.user)
...
async def withdraw(ctx, amount: int):
    ...
    if amount > user_eco[str(ctx.author.id)]["Deposited"]:
        withdraw_fail_embed = discord.Embed(title="⚠️ You do not have sufficient funds in your bank to withdraw this amount", description=None, color=discord.Color.red())
    ...
    if str(ctx.author.id) in bank:
        if user_eco[str(ctx.author.id)]["Balance"] >= 0:
            user_eco[str(ctx.author.id)]["Bankrupt"] = "Not Bankrupt"
...
#reset bank account
@client.hybrid_command(name="reset_bank_account", description="resets a users bank account to a value", aliases=["rb"])
async def resetbank(ctx, member: discord.Member, current_bal: str):
    with open("db/json/eco.json", "r") as f:
        user_eco = json.load(f)
    with open("db/json/bank.json", "r")as f:
        bank = json.load(f)
    ...
    if str(member.id) in bank:

        bank.pop(str(member.id))

        user_eco[str(ctx.author.id)] = {}
        user_eco[str(ctx.author.id)]["Balance"] = current_bal
        user_eco[str(ctx.author.id)]["Deposited"] = 0
        user_eco[str(ctx.author.id)]["Bankrupt"] = "Not Bankrupt"
        user_eco[str(ctx.author.id)]["Deposit Limit"] = 100
        ...
        await member.send("your bank account for the gamers of glory sever has been reset")
        await ctx.send("reset")
    else:
        await ctx.send("That person is not bankrupt")
...
async def bankfee():
    while True:
        ...
        for member_id, data in user_eco.items():
            dp_limit = data["Deposit Limit"]
            dp_value = data["Deposited"]
            cb_value = data["Balance"]

            if dp_value > dp_limit:
                cb_value -= 10
                user = await client.fetch_user(int(member_id))
                
                dp_limit_embed = discord.Embed(title="⚠️Deposit Limit Breach⚠️", description="Your current balance has been ducted by $5", color=discord.Color.red())
                ...
                dp_limit_embed.add_field(name="You can incress you deposit limit by using", value="=shop dp_limit",inline=False)
                dp_limit_embed.add_field(name="Your current deposit limit is", value=dp_limit, inline=False)

                await user.send(embed=dp_limit_embed)

                # Make sure balance doesn't go below -100
                cb_value = max(cb_value, -200)

                data["Balance"] = cb_value

                if cb_value <= -100:
                    # Save the user's ID with negative balance to the 'bank.json' file
                    with open("db/json/bank.json", "r") as nb_file:
                        negative_balances = json.load(nb_file)
                    
                    negative_balances[str(member_id)] = {} 
                    user_eco[str(member_id)]["Bankrupt"] = "Bankrupt"
                    
                    with open("db/json/bank.json", "w") as nb_file:
                        json.dump(negative_balances, nb_file, indent=4)
                    with open("db/json/eco.json", "w") as f:
                        json.dump(user_eco, f, indent=4)                   
...
async def shop(ctx):
    ...
    shop_embed.add_field(name="Exculsive role (=shop ex_role ROLE_NAME)", value="$15,000")
...
async def on_guild_join(guild):
    ...
    with open("db/json/welcome.json", "w") as f:
...
# main commands ______________________________________________________________________

#mute role
@client.tree.command(name="set-timeout-role", description='sets a role for the mute/timeout command.')
@commands.has_permissions(administrator=True)
async def settimeoutrole(interaction: discord.Interaction, role: discord.Role):
    with open("db/json/mute.json", "r") as f:
        mute_role = json.load(f)
        
        mute_role[str(interaction.guild.id)] = role.name
    
    with open("db/json/mute.json", "w") as f:
        json.dump(mute_role, f, indent=4)

    config_embed = discord.Embed(title="Success!", description=f"The timeout/mute role has been set to {role.mention}.", colour=discord.Colour.green())
    await interaction.response.send_message(embed=config_embed, ephemeral=True)

#mute
@client.tree.command(name="timeout-mute", description="stops the user from communicating")
@commands.has_permissions(administrator=True)
async def timeout(interaction: discord.Interaction, member: discord.Member):
    with open("db/json/mute.json", "r") as f:
        role = json.load(f)

    if role[str(interaction.guild.id)] == None:
        return await interaction.response.send_message("You do not have a mute role set.", ephemeral=True)
    else:
        mute_role = discord.utils.get(interaction.guild.roles, name=role[str(interaction.guild.id)])

        await member.add_roles(mute_role)

        config_embed = discord.Embed(title="Timeout", description=f"{interaction.user.mention} has timed out / Muted {member.mention}.", colour=discord.Colour.green())
        await interaction.response.send_message(embed=config_embed)
        
#unmute
@client.tree.command(name="untimeout-unmute", description="allows the user to communicate again.")
@commands.has_permissions(administrator=True)
async def untimeout(interaction: discord.Interaction, member: discord.Member):
    ...
    config_embed = discord.Embed(title="unTimeout", description=f"{interaction.user.mention} has untimed out / unMuted {member.mention}.", colour=discord.Colour.green())
    await interaction.response.send_message(embed=config_embed)
...
#latency 
@client.hybrid_command(name="lantency", description="shows bot's lantency in ms.", aliases=["ping", "lat"])
